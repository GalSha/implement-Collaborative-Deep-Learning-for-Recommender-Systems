...
def evaluate_model(mat, testRatings, testNegatives, K, num_thread):
    """
...
def eval_one_rating(idx):
    ...
    #predictions = _mat[user][items]


    for i in range(len(items)):
        item = items[i]
        map_item_score[item] = _mat[u,item]
    items.pop()
...
def eval_recall_precision(idx):
    ...
    for i in range(len(items)):
        map_item_score[items[i]] = _mat[u,items[i]]
